...
export function EphemerisPanel() {
  ...
  const { vectors, rawResult, isLoading, isError, error, refetch } =
    useHorizonsEphemeris(selectedNeoId, startDate, endDate);
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && clearSelected()}>
      <SheetContent
        ...
        className="bg-background border-l border-border overflow-y-auto p-0 transition-none"
      >
        {/* Drag Handle */}
        <div
          className="absolute left-0 top-0 bottom-0 w-2 cursor-col-resize hover:bg-muted active:bg-muted/80 transition-colors z-50 flex items-center justify-center group"
          onPointerDown={startResizing}
        >
          <div className="h-12 w-0.5 bg-border rounded-md group-hover:bg-foreground/20 transition-colors" />
        </div>
        ...
        <div className="mt-4 space-y-4">
          ...
          {/* Error state */}
          {isError && (
            <div className="rounded-none border-2 border-red-600 bg-red-50 text-red-600 p-4 dark:bg-black dark:border-red-600">
          ...
          {!isLoading && rawResult && (
            <details className="group">
              ...
              <div className="mt-2 rounded-none border-2 border-foreground bg-background overflow-x-auto">
